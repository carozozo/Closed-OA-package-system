...
tl.string = (function () {
    ...
    var changeCase = function (str, type, opt) {
        ...
        var force = true;
        if (opt) {
            start = !tl.helper.isEmptyVal(opt.start) ? opt.start : start;
            end = opt.end || end;
            force = opt.force !== false;
        }
        if (!tl.helper.isStr(str)) {
            if (!force) {
                return str;
            }
            str = '';
        ...
        if (tl.helper.isEmptyVal(start)) {
            return str[type]();
        }
        var ret = [];
        start = tl.helper.coverToInt(start);
        end = tl.helper.coverToInt(end);
    ...
    self.isEngLetter = function (str) {
        if (!tl.helper.isStr(str)) return false;
    ...
    self.addHead = function (str, addStr) {
        if (!tl.helper.isStr(str)) return str;
    ...
    self.formatMoney = function (str, arg1, arg2) {
        var ret = [];
        var isObj = tl.helper.isObj;
        var isStr = tl.helper.isStr;
    ...
    self.splitStr = function (str, splitter, opt) {
        ...
        splitter = tl.helper.isStr(splitter) ? splitter : '';
