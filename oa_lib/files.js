...
tl.files = (function () {
    ...
    self.isFullPath = function (path) {
        ...
        return (path.indexOf(tg.serverRoot) === 0);
    };
    ...
    self.readDir = function (rootPath, opt) {
        ...
        var readDir = function (path, opt, firstRootPath, levelCount) {
            if (!tl.helper.isStr(path) || path === '') return [];
            ...
            if (opt) {
                ...
                if (getFile) {
                    getByExtend = (tl.helper.isStr(opt.getByExtend)) ? opt.getByExtend : false;
                    if (getByExtend) {
                        getByExtend = tl.string.addHead(getByExtend, '.');
                    }
                }
            }
            ...
            tl.object.eachObj(files, function (i, file) {
    ...
    self.unlink = function (path) {
        ...
        try {
            ...
            tl.console.logErr('tl.files.unlink', e, {
                path: path
            });
            return false;
        }
     ...
     *
     * EX1
     * tl.files.unlinkUnderDir('aa/bb/cc', {
    ...
    self.unlinkUnderDir = function (path, opt) {
        ...
        keepByExtname = tl.string.splitStr(keepByExtname, ',');
        removeByExtname = tl.string.splitStr(removeByExtname, ',');
        path = self.coverToFullPath(path);
        try {
            var files = nFs.readdirSync(path);
            tl.object.eachObj(files, function (i, file) {
                ...
                if (self.isDir(curPath)) {
                    // recursively
                    if (tl.helper.isEmptyVal(removeByExtname)) {
                        opt.removeSelf = true;
                    }
                ...
                if (!tl.helper.isEmptyVal(keepByExtname)) {
            ...
            tl.console.logErr('tl.files.unlinkUnderDir', e, {
                path: path,
                opt: opt
            });
            return false;
    ...
    self.createDir = function (path) {
        path = self.coverToFullPath(path);
        var aPath = tl.string.splitStr(path, '\\');
        var subPath = '';
        tl.object.eachObj(aPath, function (i, eachDir) {
