...
tl.object = (function () {
    ...
    var changeStrValByObjKey = function (obj, type, aKey, opt) {
        ...
        if (!tl.helper.isObj(obj) || aType.indexOf(type) < 0) {
            return obj;
        }
        var objRet = obj;
        var useClone = false;
    ...
    self.trimObjVal = function (obj, opt) {
        ...
        self.eachObj(objRet, function (key, val) {
            if (tl.helper.isObj(val)) {
                objRet[key] = self.trimObjVal(val, opt);
            }
            if (tl.helper.isStr(val)) {
                objRet[key] = val.trim();
            }
        });
        return objRet;
    ...
    self.keyInObj = function (obj, aKey) {
        var pass = true;
        aKey = tl.helper.coverToArr(aKey);
        self.eachObj(aKey, function (i, key) {
